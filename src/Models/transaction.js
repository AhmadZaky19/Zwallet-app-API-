  ...
  transaction: (body) => {
    return new Promise((resolve, reject) => {
      ...
      db.query(queryTransaction, [body, body.sender_id], (err, resultData) => {
        if (err) {
          reject({
            msg: `Transaction failed. error: insert trasnsaction(${err})`,
          });
          console.error(err);
        }
        // decrease balance of sender
        const queryBalance =
          "SELECT username, balance from tb_user JOIN tb_balance ON tb_user.user_id=tb_balance.user_id WHERE tb_balance.user_id=?;";
        db.query(queryBalance, [body.sender_id], (err, dataSender) => {
          if (err) {
            reject({
              msg: `Transaction failed. error: select balance(${err})`,
            });
            console.error(err);
          }
          const queryUpdateBalance =
            "UPDATE tb_balance SET balance=? WHERE user_id=?;";
          ...
          db.query(
            ...
            (err, res) => {
              if (err) {
                reject({
                  msg: `Transaction failed. error: update balance(${err})`,
                });
                console.error(err);
              }
              ...
              db.query(
                ...
                (err, dataReceiver) => {
                  ...
                  newBalance =
                    Number(dataReceiver[0].balance) + Number(body.amount);
                  db.query(
                    queryUpdateBalance,
